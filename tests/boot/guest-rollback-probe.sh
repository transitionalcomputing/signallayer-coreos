#!/usr/bin/bash
# Temporary Phase 3E lifecycle probe supplied through a systemd credential.
set -uo pipefail
export LC_ALL=C SYSTEMD_PAGER=cat
exec 3>/dev/virtio-ports/org.signallayer.phase3e-test
...
start_proxy() {
    ...
    cat >/etc/containers/registries.conf.d/99-phase3e-test.conf <<'REGISTRY'
[[registry]]
location = "localhost"
insecure = true
REGISTRY
    cat >/run/phase3e-proxy.py <<'PY'
...
PY
    python3 /run/phase3e-proxy.py >/run/phase3e-proxy.log 2>&1 &
    for ((i=0; i<100; i++)); do
...
state_dir=/var/lib/signallayer-phase3e
wait_ready
mkdir -p "$state_dir"
...
if [[ ! -e $state_dir/rollback-reboot-requested ]]; then
    ...
    collect rollback_invocation_before systemctl show sl-rollback.service --property=InvocationID --value
    collect unprivileged_rollback runuser -u nobody -- corectl rollback --json
    collect rollback_invocation_after_unprivileged systemctl show sl-rollback.service --property=InvocationID --value
    collect malformed_rollback busctl call org.signallayer.Platform1 /org/signallayer/Platform1 org.signallayer.Platform1 StartRollback s unexpected
    collect rollback_invocation_after_malformed systemctl show sl-rollback.service --property=InvocationID --value

    monitor_worker sl-rollback.service '/usr/bin/bootc rollback' /run/phase3e-rollback-process &
    monitor_pid=$!
    start_ns=$(date +%s%N)
    collect start_rollback corectl rollback --json
    end_ns=$(date +%s%N)
    record start_rollback_elapsed_ns "$((end_ns-start_ns))"
    collect conflicting_rollback corectl rollback --json
    collect conflicting_update corectl update --json
    collect rollback_wait wait_for_unit sl-rollback.service 120
    wait "$monitor_pid" || true
    collect rollback_process cat /run/phase3e-rollback-process
