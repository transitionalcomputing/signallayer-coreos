...
source_ref=${1:-localhost/signallayer-coreos:0.0.1}
if (( $# > 1 )); then
    echo "Usage: $0 [local CoreOS OCI reference]" >&2
    exit 2
fi
mkdir -p "$build_dir/output"
run_dir=$(mktemp -d "$build_dir/output/coreos-0.0.1-x86_64.XXXXXXXX")
...
main() {
    ...
    [[ $(podman image inspect "$source_id" --format '{{index .Labels "org.opencontainers.image.version"}}') == 0.0.1 ]] || { echo 'Source must be CoreOS 0.0.1.'; exit 1; }
    ...
    podman run --rm --privileged --security-opt label=disable \
        ...
        podman tag "$expected_id" localhost/signallayer-coreos:0.0.1
        actual_id=$(podman image inspect localhost/signallayer-coreos:0.0.1 --format "{{.Id}}")
        [[ ${actual_id#sha256:} == ${expected_id#sha256:} ]]
        printf "Builder payload local ID: %s\n" "$actual_id"
        podman run --rm --entrypoint /usr/bin/bootc localhost/signallayer-coreos:0.0.1 container lint
